...
add_action( 'rest_api_init', 'WSU\Events_Calendar\Extended\register_api_fields' );
add_filter( 'tribe_events_pro_recurrence_batch_size', 'WSU\Events_Calendar\Extended\limit_recurring_batch_size', 10 );
add_filter( 'tribe_events_pro_recurrence_processor_interval', 'WSU\Events_Calendar\Extended\set_recurrence_cron_interval' );
add_filter( 'tribe_events_register_event_type_args', 'WSU\Events_Calendar\Extended\register_events_endpoint' );
add_action( 'admin_init', 'WSU\Events_Calendar\Extended\remove_events_calendar_actions', 9 );
add_action( 'plugins_loaded', 'WSU\Events_Calendar\Extended\remove_events_calendar_app_shop', 11 );
add_action( 'init', 'WSU\Events_Calendar\Extended\add_university_taxonomies', 12 );
add_filter( 'rest_tribe_events_query', 'WSU\Events_Calendar\Extended\filter_rest_query', 10, 1 );
add_action( 'tribe_settings_tab_fields', 'WSU\Events_Calendar\Extended\add_title_fields', 10, 2 );
add_filter( 'spine_sub_header_default', 'WSU\Events_Calendar\Extended\spine_sub_header' );
add_filter( 'tribe_events_show_licenses_tab', '__return_false' );
add_filter( 'Tribe__Events__Pro__Recurrence_Meta_getRecurrenceMeta', 'WSU\Events_Calendar\Extended\fix_missing_exclusions' );
add_action( 'tribe_settings_tab_fields', 'WSU\Events_Calendar\Extended\add_custom_community_settings', 10, 2 );
add_action( 'tribe_community_events_form_errors', 'WSU\Events_Calendar\Extended\community_events_submission_details' );
add_filter( 'tribe_aggregator_origins', 'WSU\Events_Calendar\Extended\filter_import_origins' );
 ...
 * @return int
 */
function limit_recurring_batch_size() {
	return 5;
}
 ...
 * @return string
 */
function set_recurrence_cron_interval() {
	return 'twicedaily';
}
 ...
 * @return array
 */
function register_events_endpoint( $args ) {
...
function register_api_fields() {
	$args = array(
		'get_callback' => 'WSU\Events_Calendar\Extended\get_api_meta_data',
 ...
 */
function remove_events_calendar_actions() {
 ...
 */
function remove_events_calendar_app_shop() {
 ...
 * @return array
 */
function add_title_fields( $settings, $id ) {
	if ( 'general' === $id ) {
 ...
 */
function spine_sub_header( $sub_header_default ) {
 ...
 * @return array
 */
function fix_missing_exclusions( $recurrence_meta ) {
 ...
 * @return array
 */
function add_custom_community_settings( $settings, $id ) {
 ...
 * @return mixed
 */
function community_events_submission_details( $errors ) {
 ...
 * @return array
 */
function filter_import_origins() {
	return array(
		'csv' => (object) array(
			'id' => 'csv',
			'name' => 'CSV File',
